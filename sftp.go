...
func (c *Client) UploadIfNewer(src string, dest string) (bool, error) {
	srcinfo, err := os.Lstat(src)
	if err != nil {
		return false, err
	}

	dstinfo, err := c.Info(dest)
	if err == nil {
	...
	return true, c.UploadFile(src, dest)
}

func (c *Client) UploadFile(src string, dest string) error {
